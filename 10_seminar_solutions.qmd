...
y <- 20L
as.raw(y) # hex
as.integer(y) # decimal
paste(rev(as.integer(rawToBits(as.raw(y)))), collapse = "") # binary
```
...
```{r}
length(raw_bytes) # number of bytes in the file
file.info(paste0(ddir, "/pubs_coords.csv"))$size # the same number, from disk
```
...
```{r}
nchar(encoded) # 40 characters...
length(jpg_bytes[1:30]) # ...to store 30 bytes, so about 33% larger
identical(base64_decode(encoded), jpg_bytes[1:30]) # TRUE: it round-trips
```
...
Because the file is likely being read or written as if it were UTF‑8, but it’s actually encoded in UTF‑16 or UTF‑32.

Each character in UTF‑16/32 uses 2 or 4 bytes, while UTF‑8 uses 1–4 bytes per character. If the program assumes UTF‑8 but the file is UTF‑16/32, it misinterprets the byte sequences — leading to lost or garbled characters.
:::
...
rf_accuracy <- mean(predictions == test_y)
...
Not too bad if you consider that we only used trivial color features! We got `r round(rf_accuracy*100, 1)`% right, with very little effort.
...
# now add the pubs
basemap +
    geom_sf(data = pubs_sf, color = "red", size = 0.5) +
    labs(title = "Historic Pubs in London", subtitle = "Red dots indicate pubs")
```
...
pubs_sf <- st_join(pubs_sf, london_sf, join = st_within) # join the pubs to the boroughs
pubs_sf
```
...
pub_density <- pubs_sf %>%
    ...
    summarise(
        pub_count = n(),
        area = mean(area), # this is a constant within each borough
        pub_density = pub_count / (area / 1000000) # pub density in pubs per square kilometer
    ) %>%
    ungroup() %>%
    st_drop_geometry() # drop the geometry column as we don't need it anymore
...
```{r merge_density}
# merge with the london_sf data based on the BOROUGH column
london_sf <- london_sf %>%
    left_join(pub_density, by = c("BOROUGH", "area"))
...
```{r plot_density}
# if there was no pub at all, it will be NA, so we can replace that with 0
london_sf <- london_sf %>%
    mutate(pub_density = ifelse(is.na(pub_count), 0, pub_density)) # replace NA with 0

# Let's plot the ultimate pub density map
ggplot(london_sf) +
    geom_sf(aes(fill = as.numeric(pub_density)), color = "white") +
    scale_fill_viridis_c(option = "C", name = "Pub Density (per km²)") +
    labs(title = "Historic Pubs per Square Kilometer", x = "Longitude", y = "Latitude") +
    theme_bw() +
...
```{r, eval = FALSE}
2^16 # 65536 distinct amplitude levels
1 / mlk@samp.rate # seconds between successive samples
```
...
```{r, eval = FALSE}
tp <- tibble(
    seconds = (1:length(mlk.clip@left)) / mlk.clip@samp.rate,
...
# zoom in to the first 110 samples
ggplot(tp[1:110, ], aes(x = seconds, y = amplitude)) +
    geom_line() +
    theme_minimal()
```
