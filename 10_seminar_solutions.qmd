...
root <- "/Users/anton/Library/CloudStorage/OneDrive-LondonSchoolofEconomics/Teaching/2025/ME314_prep/labs/seminar10"
...
```{r}
# select all tables with the class "wikitable.sortable"
all_pubs <- pubs_html |>
...
```{r}
# This is how you get the names of the pubs
pub_urls <- pubs_html |>
...
Finally, let's combine the two cilumns iunto a dataframe.
...
```{r}
# extract the coordinates from the html
pub_coords <- pub_html |>
    # note: html_element() (without "s") returns a single element, html_elements() returns a list of elements
...
Note, we now have the BOROUGH information in the pub data. `st_join()` simply looked at the coordinates and the shapes and used the `st_within` operation to locate the pubs in the boroughs. We can now 
...
```{r}
content$items[[1]]$value$nameListAs
content$items[[1]]$value$latestParty$name

# also interesting: the metadata
content$totalResults # number of MPs (more than expected)
content$take # number of results per page
```
...
mp_df <- mp_list |>
    ...
    # still deeply nested, so we need to unnest the next level
    # let's ignore the links for now
    tidyr::unnest_wider(value) |>
    # because there is an `id` column in latestParty and we already have such a column, we need to unnest this but tell R how to uniquely identify the columns
    tidyr::unnest_wider(latestParty, names_sep = "_") |>
    # finally, we only want one entry, the link to contact info from the links column. We can do this as using the map and pluck functions
    mutate(contact_url = map_chr(
        links,
        # for each row, go to the fourth list and take the second element from that list
        ~ pluck(.x, 4, 2,
            # return a character vector, if there is no such element, return NA
            .default = NA_character_
        )
    )) |>
    select(-links, -latestHouseMembership) # we don't need these for now

# nice, rectangular and tidy data frame
glimpse(mp_df)
```
