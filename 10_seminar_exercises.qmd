...
Let's inspect the html. It doesn't print too nicely but we can convert it to a list and then inspect it.

```{r, eval=FALSE}
print(pubs_html)
str(xml2::as_list(pubs_html), max.level = 2)
```
...
```{r}
# grab the first data table by class, not by position
...
str(first_pub_df) # already a single tibble, no list wrapper to unnest
```
...
Now that we have each pubs wikipedia page, we could scrape the coordinates from there. To do that, we would go to each page (e.g. <https://en.wikipedia.org/wiki/Ship_Tavern,_Holborn>), download the html, and extract the coordinates. 
...
for (i in start_index:end_index) {
    ...
    if (i <= length(pub_coords_list) && !is.null(pub_coords_list[[i]])) {
...
etiquette <- function(req) {
    req <- req %>%
        ...
        req_retry(
            # try again up to 3 times if the server returns a rate limit or service down error
            max_tries = 3,
...
Please do not run the code below, it is just pseudo-code to illustrate how you would loop through the pages of the API. We have already downloaded the data for you and saved it in the `data/mps` directory as `tories_untidy.json` and `labour_untidy.json`.
:::
...
# start at the first result (the API returns results in pages of 20)
skip <- 0
...
If you didn't manage to create the dataframe with the MPs, you can use the `mps.csv` file in the `data/mps` directory. It contains all the information we need to download the images.
