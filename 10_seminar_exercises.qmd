...
setwd("/Users/charlottekuberka/Desktop/ME314/2026_versions/Seminar10")
...
if (all(required_data %in% list.files(ddir))) {
  ...
  cat(
    "You have misspecified the directory. Please read the instructions above carefully and try again.\n"
  )
}
```
...
When you create an object in R, you are making digital data. R shows it to you in a friendly format (R interprets it for us already and provides the data in a readable format ), but you can ask for the underlying bytes.

```{r}
x <- 5L # an integer object
as.raw(x) # the byte, written as a hex code (minimal R interpretation)
...
extract_image_features <- function(img_path) {
  ...
  features <- list(
    ...
    mean_red_saturation = mean(ifelse(
      r == pmax(r, g, b),
    ...
    red_top_quartile_mean = mean(sort(r, decreasing = TRUE)[
      1:floor(length(r) * 0.25)
    ]),
...
image_df <- image_features %>%
  ...
  mutate(
    id = mp_df$id,
    name = mp_df$nameDisplayAs,
    party = mp_df$latestParty_name
  )
```
...
image_df$labour <- ifelse(
  image_df$party == "Labour" |
    image_df$party == "Labour (Co-op)",
  1,
  0
)

image_df$labour <- factor(
  image_df$labour,
  levels = c(0, 1),
...
# let's compare the structures of both data objects
...
london_sf <- sf::read_sf(paste0(
  ddir,
  "/London_Boroughs/London_Boroughs.shp"
)) %>%
...
Now add the pubs to the map using `geom_sf()` and the `pubs_sf` object we created earlier. Simply add a new `geom_sf()` layer, specifying the `data` argument to be `pubs_sf`, and set the color to "red" and size of the points to 0.5. You don't need to specify any `aes()` mappings.
...
ggplot(london_sf) +
  ...
  labs(
    title = "Historic Pubs per Square Kilometer",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_bw() +
